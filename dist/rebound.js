...
function interpolateColor(val, startColorStr, endColorStr) {
  ...
  var r = mapValueInRange(val, fromLow, fromHigh, startColor.r, endColor.r) | 0;
  var g = mapValueInRange(val, fromLow, fromHigh, startColor.g, endColor.g) | 0;
  var b = mapValueInRange(val, fromLow, fromHigh, startColor.b, endColor.b) | 0;
  if (asRGB) {
    return 'rgb(' + r + ',' + g + ',' + b + ')';
  } else {
...
var Spring = function () {
  ...
  Spring.prototype.setEndValue = function setEndValue(endValue) {
    ...
    for (var i = 0, len = this.listeners.length; i < len; i = i + 1) {
  ...
  Spring.prototype.advance = function advance(time, realDeltaTime) {
    ...
    var SOLVER_TIMESTEP_SEC = Spring.SOLVER_TIMESTEP_SEC,
        SOLVER_TIMESTEP_SEC_BY_2 = Spring.SOLVER_TIMESTEP_SEC_BY_2;
    ...
    while (this._timeAccumulator >= SOLVER_TIMESTEP_SEC) {
      ...
      dvdt = ONE_BY_6 * (aAcceleration + 2.0 * (bAcceleration + cAcceleration) + dAcceleration);
...
Spring.SOLVER_TIMESTEP_SEC_BY_2 = 0.001 * 0.5;
...
var SpringSystem = function () {
  ...
  SpringSystem.prototype.advance = function advance(time, deltaTime) {
    ...
    for (var i = 0, len = this._activeSprings.length; i < len; i = i + 1) {
      var spring = this._activeSprings[i];
      if (spring.systemShouldAdvance()) {
        spring.advance(time, deltaTime);
      } else {
        this._idleSpringIndices[idleSpringCount++] = this._activeSprings.indexOf(spring);
      }
    }
    while (idleSpringCount) {
      var idx = this._idleSpringIndices[--idleSpringCount];
      this._idleSpringIndices.length = idleSpringCount;
  ...
  SpringSystem.prototype.activateSpring = function activateSpring(springId) {
    ...
    var _activeSprings = this._activeSprings;
